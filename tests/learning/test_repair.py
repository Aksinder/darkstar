...
def test_hourly_kwh_mean_and_change():
    stats = {
        "sensor.solpaneler": [{"start": _epoch_ms(HOUR), "mean": 8000.0}],
        "sensor.total_imported_energy": [{"start": _epoch_ms(HOUR), "change": 1.2}],
    }
    assert _hourly_kwh(stats, "sensor.solpaneler", "mean_w") == {HOUR: 8.0}
    assert _hourly_kwh(stats, "sensor.total_imported_energy", "change") == {HOUR: 1.2}
    assert _hourly_kwh(stats, None, "mean_w") == {}
...
def test_compute_slot_record_balance_and_quarter_split():
    ...
    rec = compute_slot_record(HOUR + timedelta(minutes=15), hourly)
...
async def test_repair_targets_artifacts_only_and_respects_dry_run(monkeypatch):
    ...
    rows = [
        ...
        {"slot_start": start.astimezone(tz).isoformat(), "load_kwh": 0.4},
        # slot :15 artifact (load 0.0) -> repaired
        {"slot_start": (start + timedelta(minutes=15)).astimezone(tz).isoformat(), "load_kwh": 0.0},
    ...
    async def fake_stats(entity_ids, s, e, **kw):
        return {
            "sensor.solpaneler": [{"start": _epoch_ms(HOUR), "mean": 4000.0}],
